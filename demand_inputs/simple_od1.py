...
def map_parcels_to_nodes():
    ### read bolinas nodes
    nodes_df = pd.read_csv(absolute_path + '../network_inputs/osm_nodes_woodacre.csv')
    nodes_gdf = gpd.GeoDataFrame(nodes_df, geometry=[shapely.geometry.Point(xy) for xy in zip(nodes_df.y, nodes_df.x)], crs='epsg:4326')
...
def generate_simple_od(vphh=1, visitor_cnts=0):
    ...
    household_parcels = map_parcels_to_nodes()
    
    ### local residents origin
    local_residents_od_df = household_parcels[['OBJECTID', 'closest_node']].copy().sample(frac=vphh,replace=True).copy().reset_index(drop=True)
...
#     visitor_vehicle_od_df['Parcel'] = None
    ...
    od_df = pd.concat([local_residents_od_df] 
